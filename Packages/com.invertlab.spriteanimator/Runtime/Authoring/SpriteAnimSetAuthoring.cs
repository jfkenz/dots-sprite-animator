    ...
    public class SpriteAnimSetAuthoring : MonoBehaviour
    {
        ...
        class Baker : Baker<SpriteAnimSetAuthoring>
        {
            public override void Bake(SpriteAnimSetAuthoring authoring)
            {
                ...
                {
                    if (useProfile && profile.SheetForClip(profile.Clips[i])?.Texture != null)
                        DependsOn(profile.SheetForClip(profile.Clips[i]).Texture);
                    inputs[i] = SpriteAnimClipConversion.CreateInput(authoring, i);
                }
